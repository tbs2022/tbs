...
inline EdgeWeight graph_access::getEdgeCut(){
#ifdef NDEBUG
    EdgeWeight total = 0;
    for (int i = 0; i < graphref->e; ++i) {
        total += graphref->m_edges[i].weight;
    }
    return total / 2;
#else
    EdgeWeight total = 0;
    for (int i = 0; i < graphref->e; ++i) {
        total += graphref->m_edges.at(i).weight;
    }
    return total / 2;
#endif
}
