...
void partitionAdjMat::pairwise_cut(graph_access & G) {
    assert(G.get_partition_count() == this->n);
//    std::fill(&edgecuts[0][0], &edgecuts[0][0] + sizeof(edgecuts) / sizeof(edgecuts[0][0]), 0);
    forall_nodes(G, n) {
//                int edgeCut = 0;
                ...
                forall_out_edges(G, e, n) {
                            ...
                            if (partitionIDSource != partitionIDTarget) {
                                adjMat[partitionIDSource][partitionIDTarget] += G.getEdgeWeight(e);
// two virtual edges for one edge make sure this happens, no need to write this line
//                                edgecuts[partitionIDTarget][partitionIDSource] += G.getEdgeWeight(e);
                            }
                        } endfor
            } endfor
}
...
EdgeWeight quality_metrics::max_pairwise_cut(graph_access & G) {
    EdgeWeight maxedgeCut = 0;
    int edgecuts[G.get_partition_count()][G.get_partition_count()];
    std::fill(&edgecuts[0][0], &edgecuts[0][0] + sizeof(edgecuts) / sizeof(edgecuts[0][0]), 0);
    forall_nodes(G, n) {
//                int edgeCut = 0;
                PartitionID partitionIDSource = G.getPartitionIndex(n);
                forall_out_edges(G, e, n) {
                            ...
                            if (partitionIDSource != partitionIDTarget) {
                                edgecuts[partitionIDSource][partitionIDTarget] += G.getEdgeWeight(e);
// two virtual edges for one edge make sure this happens, no need to write this line
//                                edgecuts[partitionIDTarget][partitionIDSource] += G.getEdgeWeight(e);
                            }
                } endfor
    } endfor
    for (int i = 0; i < G.get_partition_count(); ++i) {
        for (int j = 0; j < G.get_partition_count(); ++j) {
            maxedgeCut = std::max(maxedgeCut, edgecuts[i][j]);
        }
    }
    return maxedgeCut;
}
